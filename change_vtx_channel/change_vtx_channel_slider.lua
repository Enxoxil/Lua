...
local function initialize_parameters()
	...
	frequencies[4] = param:get('SCR_USER4');
	frequencies[5] = param:get('SCR_USER5');

	aux_for_change_VTX_channel = param:get('SCR_USER6');

	-- Check if parameters are loaded
	if not frequencies[1] or not frequencies[2] or not frequencies[3] or not frequencies[4] or not frequencies[5] or not aux_for_change_VTX_channel  then
